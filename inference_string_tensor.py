...
class TFObjectDetector:
    def __init__(self, path_to_model):
        ...
        with self.detection_graph.as_default():
            ...
            self.image_tensor = self.detection_graph.get_tensor_by_name('encoded_image_string_tensor:0')
...
def object_detection(im_path, detector, threshold, input_tensor_type):
    img = cv2.imread(im_path)
    height, width = img.shape[:2]
    
    if input_tensor_type == 'nparray':
        img_expanded = np.expand_dims(img, 0)
        boxes, scores, classes, num = detector.detect(img_expanded)
    elif input_tensor_type == 'string':
        with open(im_path, 'rb') as f:
            buf = f.read()
        #encoded_image = [base64.b64encode(buf).decode('utf-8')]
        encoded_image = [buf]
        boxes, scores, classes, num = detector.detect(encoded_image)
    else:
        print('input_tensor_type must be one of (nparray, string)')
...
def parse_args():
    ...
    parser.add_argument('--input_tensor_type', required=True, help='one of (string, nparray)')
...
if __name__ == '__main__':
    ...
    for im_path in test_images:
        ...
        start_od = time.time()
        inference = object_detection(im_path, detector, args.threshold, args.input_tensor_type)
